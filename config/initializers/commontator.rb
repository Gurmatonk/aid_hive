...
Commontator.configure do |config|
  ...
  config.user_link_proc = -> (user, _app_routes) { Rails.application.routes.url_helpers.user_path(user) }
  ...
  #              user, 1, :s => 60, :d => 'mm') }
  config.user_avatar_proc = -> (user, view) { view.commontator_gravatar_image_tag(user, 1, s: 60, d: 'mm') }
  ...
  config.email_from_proc = -> (_thread) { 'no-reply@aid-hive.org' }
  ...
  config.commontable_name_proc = -> (thread) { "#{thread.commontable.class.name} ##{thread.commontable.id}" }
  ...
  config.comment_url_proc = -> (comment, app_routes) { app_routes.polymorphic_url(comment.thread.commontable, anchor: "comment_#{comment.id}_div") }
  ...
  config.user_mentions_proc = -> (current_user, query) { current_user.class.where('name ILIKE ?', "#{query}%") }
end
