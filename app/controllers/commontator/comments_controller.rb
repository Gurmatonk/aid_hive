module Commontator
  class CommentsController < Commontator::ApplicationController
    ...
    def undelete
      security_transgression_unless @comment.can_be_deleted_by?(@user)
      @comment.errors.add(:base, t('commontator.comment.errors.not_deleted')) unless @comment.undelete_by(@user)
    ...
    def downvote
      security_transgression_unless @comment.can_be_voted_on_by?(@user) && @comment.thread.config.comment_voting.to_sym == :ld
